            ...
            <form action="{{ route('mahasiswa.logbook.update', ['id' => $logbook->id]) }}" method="POST">
                ...
                <div class="mb-4">
                    ...
                    <textarea id="logbook" name="logbook" class="w-full px-4 py-2 border border-gray-300 rounded-md"
                        required>{{ $logbook->logbook }}</textarea>
                </div>

                <div class="mb-4">
                    <label for="Catatan" class="block text-gray-700 font-semibold mb-2">Catatan</label>
                    <textarea id="Catatan" name="Catatan" class="w-full px-4 py-2 border border-gray-300 rounded-md"
                        readonly disabled>{{ $logbook->Catatan }}</textarea>
                </div>
